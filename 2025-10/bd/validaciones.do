gen valid_fecha_nac = !regexm(fecha_nac, "^[0-9]{1,2}/[0-9]{1,2}/[0-9]{4}$") | ///
                   (date(fecha_nac, "DMY") == .) | ///
                   missing(fecha_nac) if fecha_nac != ""
replace valid_fecha_nac = 2 if valid_fecha_nac == 0
replace valid_fecha_nac = 0 if valid_fecha_nac == 1
replace valid_fecha_nac = 1 if valid_fecha_nac == 2
				   
gen valid_fecha = !regexm(fecha, "^[0-9]{1,2}/[0-9]{1,2}/[0-9]{4}$") | ///
                   (date(fecha, "DMY") == .) | ///
                   missing(fecha) if fecha != ""
				   
replace valid_fecha = 2 if valid_fecha == 0
replace valid_fecha = 0 if valid_fecha == 1
replace valid_fecha = 1 if valid_fecha == 2
...
label variable valid_fecha_nac "Validacion fecha nacimiento"
label variable valid_fecha "Validacion fecha encuesta"

label define valid 0 "Inválido", modify
label define valid 1 "Válido", modify
label values valid_rut valid
label values valid_fecha_nac valid
label values valid_fecha valid
